...
def check_city(inp, exp):
    if inp.name != exp.name:
      ...
      return False
    if inp.lat != exp.lat:
      print("Input LAT:", inp.lat) #> looks like this is the problem somehow
      print("Expected LAT:", exp.lat)
      # Had to do this to find out the "input" is a string -__-
      print(type(inp.lat), type(exp.lat))
      return False
    if inp.lon != exp.lon:
      print("Input LON:", inp.lon)
      return False
    return True
