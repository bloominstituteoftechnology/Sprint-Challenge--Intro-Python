...
def check_city(inp, exp):
    if inp.name != exp.name:
        print(f"This is from the test file: {inp.name} {exp.name}")
        return False
    if inp.lat != exp.lat:
        print(f"This is from the test file: {inp.lat} {exp.lat} lat")
        return False
    if inp.lon != exp.lon:
        print(f"This is from the test file: {inp.name} {exp.name} lon")
        return False
    return True
