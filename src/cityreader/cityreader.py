...
def cityreader_stretch(lat1, lon1, lat2, lon2, cities=[]):
	...
	# within = [f"{city.name}: ({city.lat}, {city.lon})" for city in cities if city.lon in range(lon2, lon1) and city.lat in range(lat2, lat1)]
  	# TODO 
	# Ensure that the lat and lon valuse are all floats
  	# Go through each city and check to see if it falls within
 	# the specified coordinates.
	for city in cities:
		print(city.lon in range(lon2, lon1) and city.lat in range(lat2, lat1))
